...
export default function CodeBlock({ children, language, className, copy }: IProps) {
  ...
  return (
    <div className={`p-3 lg:p-4 xl:p-6 grid xl:py-8 my-3 bg-slate-600 text-slate-100 rounded-xl ${className}`}>
      {
        (copy || language)
        && (
          <div className="relative flex flex-row-items-center-gap-4 justify-between">
            ...
            {copy &&
              <button
                className="absolute -top-2 right-0 bg-slate-600 p-1 text-slate-100 h-max"
